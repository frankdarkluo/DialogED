...
import numpy as np
from model.hardkuma.kuma_gate import KumaGate
from model.hardkuma.util import get_encoder
from model.hardkuma.rcnn import RCNNCell
...
class DependentLatentModel(nn.Module):
    ...
    def __init__(self,hidden_size: int = 200,z_rnn_size:  int = 30):
        ...
        self.z_layer = KumaGate(hidden_size + z_rnn_size)

        self.z = None      # z samples
        self.z_dists = []  # z distribution(s)
        self.LSTM = False # False CRU # nan
        self.report_params()
    ...
    def forward(self, x, mask):
        ...
        if self.LSTM:  # LSTM
            ...
            state = h.new_zeros([3 * batch_size, self.z_rnn_size]).chunk(3)
